...
#define CHECK_CUDA(call)                                                                                          \
    {                                                                                                             \
        ...
        if (_e != cudaSuccess)                                                                                    \
        {                                                                                                         \
            std::cerr << "CUDA Runtime failure: '#" << _e <<"' " << cudaGetErrorString(_e) << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
            return EXIT_FAILURE;                                                                                     \
        }                                                                                                         \
    }
...
#define CHECK_NVJPEG2K(call)                                                                                \
    {                                                                                                       \
        ...
        {                                                                                                   \
            std::cerr << "NVJPEG failure: '#" << _e << "' " << nvjpegGetErrorString(_e) << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
            return EXIT_FAILURE;                                                                            \
        }                                                                                                   \
    }
...
#define CHECK_CUDA_NO_RETURN(call)                                                                                          \
    {                                                                                                             \
        ...
        {                                                                                                         \
            ...
            msg << "CUDA Runtime failure: '#" << _e <<"' " << cudaGetErrorString(_e) << " at " << __FILE__ << ":" << __LINE__ << std::endl; \
...
std::string nvjpegGetErrorString(nvjpeg2kStatus_t e) {
  switch(e) {
    case NVJPEG2K_STATUS_SUCCESS: return std::string("NVJPEG2K_STATUS_SUCCESS");
    case NVJPEG2K_STATUS_NOT_INITIALIZED: return std::string("NVJPEG2K_STATUS_NOT_INITIALIZED");               
    case NVJPEG2K_STATUS_INVALID_PARAMETER: return std::string("NVJPEG2K_STATUS_INVALID_PARAMETER");             
    case NVJPEG2K_STATUS_BAD_JPEG: return std::string("NVJPEG2K_STATUS_BAD_JPEG");                      
    case NVJPEG2K_STATUS_JPEG_NOT_SUPPORTED: return std::string("NVJPEG2K_STATUS_JPEG_NOT_SUPPORTED");            
    case NVJPEG2K_STATUS_ALLOCATOR_FAILURE: return std::string("NVJPEG2K_STATUS_ALLOCATOR_FAILURE");             
    case NVJPEG2K_STATUS_EXECUTION_FAILED: return std::string("NVJPEG2K_STATUS_EXECUTION_FAILED");              
    case NVJPEG2K_STATUS_ARCH_MISMATCH: return std::string("NVJPEG2K_STATUS_ARCH_MISMATCH");                 
    case NVJPEG2K_STATUS_INTERNAL_ERROR: return std::string("NVJPEG2K_STATUS_INTERNAL_ERROR");                
    case NVJPEG2K_STATUS_IMPLEMENTATION_NOT_SUPPORTED: return std::string("NVJPEG2K_STATUS_IMPLEMENTATION_NOT_SUPPORTED");  
  }
...
template <typename D>
D read(const char* buffer, size_t offset = 0) {
...
cudaError_t hostToDeviceCopy(
  ...
  const size_t copySize = cols * sizeof(D);
  if (stream == NULL) {
    return cudaMemcpy2D((D *)deviceBuf, deviceBufPitchInBytes, hostBuf, hostBufPitchInBytes, copySize, rows, cudaMemcpyHostToDevice);
  }
  else {
    return cudaMemcpy2DAsync((D *)deviceBuf, deviceBufPitchInBytes, hostBuf, hostBufPitchInBytes, copySize, rows, cudaMemcpyHostToDevice, *stream);
  }
}


template <typename D>
cudaError_t deviceMalloc(
...
int _decode_frames(
  ...
  for (size_t frameIndex = 0; frameIndex < numFrames; frameIndex++) {
    ...
    if (frameIndex % batchSize == batchSize - 1) {
      ...
      uint16_t* outBufferLastBatch = seekToFrameNumber<uint16_t>(outBuffer, cols, rows, frameIndex / batchSize * batchSize);

      // Perform the copy
      CHECK_CUDA(deviceToHostCopy<uint16_t>(devBuffer, pitchInBytes, outBufferLastBatch, params->rows * batchSize, params->cols, &params->cudaStream));
...
std::vector<unsigned char> encode(
  ...
  for (size_t frameIndex = 0; frameIndex < numFrames; frameIndex++) {
    ...
    CHECK_CUDA_NO_RETURN(hostToDeviceCopy<uint16_t>(devBufferThisFrame, pitchInBytes, hostBufferThisFrame, rows, cols));
    ...
    CHECK_NVJPEG2K_NO_RETURN(nvjpeg2kEncodeRetrieveBitstream(handle, params->encodeState, NULL, &compressed_size, NULL));
    unsigned char *compressed_data = new unsigned char [compressed_size];

    // Get the encoded data in host memory
    CHECK_NVJPEG2K_NO_RETURN(nvjpeg2kEncodeRetrieveBitstream(handle, params->encodeState, compressed_data, &compressed_size, NULL));
...
py::bytes encode_frames(
    py::array_t<uint16_t> array,
    size_t batchSize = 4
) {
  if (array.ndim() != 3) {
  ...
  // Encode all frames
  std::vector<unsigned char> encodedFrames = encode(
      buffer,
      bufferSize,
      rows,
      cols,
